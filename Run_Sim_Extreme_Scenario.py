...
INC_NT = None # {incentive_times[0]: f_inc_low, incentive_times[1]: f_inc_high}  # {incentive_times[1]: f_inc_low, incentive_times[3]: f_inc_low} # vengono influenzati 2 volte 
...
Zona_Residenziale_1 = tuple(The_Map.make_multi_frame(mx = mx, layers = 3)) # terzo anello
Zona_Residenziale_2 = tuple(The_Map.make_multi_frame(mx = mx, layers = 4)) # terzo anello
Zona_Residenziale_3 = tuple(The_Map.make_multi_frame(mx = mx, layers = 5))
...
Zona_Residenziale = Zona_Residenziale_1 + Zona_Residenziale_2 + Zona_Residenziale_3
                        
The_Map.all_equal(Estrema_Periferia, 4) # tipo Non_Green 
The_Map.all_equal(Periferia, 4) # tipo Non_Green
The_Map.all_equal(Zona_Residenziale_1, 3) # tipo Low_Green
The_Map.all_equal(Zona_Residenziale_2, 2) # tipo Neutral
The_Map.all_equal(Zona_Residenziale_3, 1) # tipo Green
The_Map.all_equal(Centro, 0) # tipo Very_Green    
...
*************************************************"""
b_cap = 500 #kg
b_tr = None # treshold di chiamata nullo, quindi a tempo fisso
...
intertempo_giro = 30*24 # 14 giorni, 2 settimane
...
QUANTITA' IN KG
*************************************************"""
seeds = (3, ) #(3, 5, 7, 9, 11) # i seed che verranno utilizzati
un_giorno = 24 # 24 ore
giorni_anno = 365
...
Excel_Files = {
                '_01 Citizens_Extreme_Case.xlsx':
                        [(DFs[0], 'Time_Series_Gaw'), (DFs[1], 'Block_Stats'), (DFs[2], 'Gaw_Components')], 
                '_01 TruckBins_Extreme_Case.xlsx':
